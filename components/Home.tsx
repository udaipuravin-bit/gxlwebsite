    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20 animate-in fade-in duration-700">
      ...
      <header className="text-center mb-20 space-y-6 relative">
        ...
        <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border text-[10px] font-black uppercase tracking-[0.2em] mb-4 ${isDark ? 'bg-indigo-500/10 border-indigo-500/20 text-indigo-400' : 'bg-indigo-50 border-indigo-100 text-indigo-600'}`}>
          <Zap size={12} className="fill-current" /> Global Domain Intelligence
        </div>
        <h1 className={`text-5xl md:text-7xl font-black tracking-tight leading-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>
          The Auditor's <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-teal-400">Control Plane</span>
        </h1>
        <p className={`text-lg md:text-xl max-w-3xl mx-auto font-medium leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
          Enterprise-grade toolkit for domain forensics, mail protocol verification, and infrastructure auditing.
        </p>
      </header>
      ...
      <section className="mb-20">
        <div className="flex items-center gap-3 mb-10">
          ...
          <h2 className={`text-2xl font-black tracking-tight uppercase ${isDark ? 'text-white' : 'text-slate-800'}`}>Authentication & Trust</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <ToolCard 
            ...
            title="Bulk DKIM" 
            desc="Verify keys across domains with custom selectors." 
          ...
          <ToolCard 
            ...
            icon={<Globe size={32} />} 
            title="SPF Validator" 
            desc="Audit DNS lookup limits and syntax errors." 
          ...
          <ToolCard 
            ...
            title="DMARC Audit" 
            desc="Check policy alignment and reporting targets." 
          ...
          <ToolCard 
            ...
            icon={<Lock size={32} />} 
            title="SSL Audit" 
            desc="Analyze certificates and expiry dates." 
      ...
      <section className="mb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <ToolCard 
            ...
            icon={<Mail size={32} />} 
            title="MX & Providers" 
            desc="Detect hosting providers and mail architecture." 
          ...
          <ToolCard 
            ...
            title="Deep DNS" 
            desc="Recursive lookup for all record types." 
          ...
          <ToolCard 
            ...
            icon={<Hash size={32} />} 
            title="Reverse DNS" 
            desc="Map IPs back to parent hostnames." 
          ...
          <ToolCard 
            ...
            title="CAA Policy" 
            desc="Restrict cert issuance authorities." 
      ...
      <section className="mb-24">
        <div className="flex items-center gap-3 mb-10">
          ...
          <h2 className={`text-2xl font-black tracking-tight uppercase ${isDark ? 'text-white' : 'text-slate-800'}`}>Forensic Lab</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
          ...
          <ToolCard 
            ...
            title="Header Forensic" 
            desc="Trace mail risk paths." 
      ...
      <footer className={`text-center pb-12 border-t pt-16 ${isDark ? 'border-slate-900' : 'border-slate-200'}`}>
        <p className="text-slate-500 text-[11px] font-bold uppercase tracking-widest">
          © {new Date().getFullYear()} Authenticator Pro • v3.0.4
        </p>
      </footer>
    </div>
  );
