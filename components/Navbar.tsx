...
const Navbar: React.FC<NavbarProps> = ({ onNavigate, theme, onToggleTheme }) => {
  ...
  const tools = {
    ...
    infrastructure: [
      ...
      { id: 'whois-checker', name: 'Expiry Date Checker', icon: <Calendar size={16} />, color: 'text-lime-400' },
    ],
    design: [
      { id: 'font-studio', name: 'Font Studio', icon: <Languages size={16} />, color: 'text-orange-500' },
      { id: 'email-builder', name: 'Email Studio', icon: <Layout size={16} />, color: 'text-indigo-400' },
  ...
  return (
    <nav className={`sticky top-0 z-[100] border-b transition-colors duration-300 ${isDark ? 'bg-slate-900/80 border-slate-800 backdrop-blur-xl' : 'bg-white/90 border-slate-200 backdrop-blur-md'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center gap-4">
          ...
          >
            <div className="bg-gradient-to-br from-orange-500 to-orange-600 p-1.5 rounded-xl shadow-lg shadow-orange-500/20 group-hover:scale-110 transition-transform">
            ...
            <span className={`text-xl font-black tracking-tighter ${isDark ? 'text-white' : 'text-slate-900'}`}>
              EMAIL <span className="text-orange-500">SPARKS</span>
            </span>
          </button>
          ...
          <div className="hidden lg:flex items-center gap-1 ml-4 flex-1">
            ...
            <div className="relative" ref={menuRef}>
              <button 
                ...
                className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${isDark ? 'text-slate-400 hover:text-white hover:bg-slate-800' : 'text-slate-600 hover:text-orange-600 hover:bg-slate-100'} ${isAllToolsOpen ? (isDark ? 'bg-slate-800 text-white' : 'bg-slate-100 text-orange-600') : ''}`}
              >
              ...
              {isAllToolsOpen && (
                <div 
                  className={`absolute top-[calc(100%+8px)] left-1/2 -translate-x-1/2 w-[880px] shadow-2xl rounded-3xl border p-8 grid grid-cols-3 gap-8 animate-in fade-in slide-in-from-top-2 duration-200 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}
          ...
          <div className="flex items-center gap-2 md:gap-4 flex-1 justify-end">
            <div className="relative w-full max-w-[120px] md:max-w-[240px]" ref={searchRef}>
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border transition-all ${
                isDark 
                  ? 'bg-slate-950 border-slate-800 focus-within:border-orange-500 focus-within:ring-1 focus-within:ring-orange-500/50' 
                  : 'bg-slate-100 border-slate-200 focus-within:border-orange-300 focus-within:ring-1 focus-within:ring-orange-200'
              }`}>
      ...
      {isMobileMenuOpen && (
        <div className={`lg:hidden fixed inset-0 z-[100] animate-in slide-in-from-right duration-300 ${isDark ? 'bg-slate-900' : 'bg-white'}`}>
          <div className="p-6 h-full flex flex-col overflow-y-auto">
            <div className="flex justify-between items-center mb-8">
              <span className={`text-xl font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>TOOLS MENU</span>
            ...
            <div className="space-y-8 pb-12">
              ...
              <div>
                <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Design</h3>
                <div className="grid grid-cols-1 gap-2">
                  {tools.design.map(t => (
                    <MegaMenuButton key={t.id} icon={t.icon} label={t.name} onClick={() => handleNavigate(t.id)} theme={theme} colorClass={t.color} />
                  ))}
...
const NavButton: React.FC<{ onClick: () => void; label: string; theme: string }> = ({ onClick, label, theme }) => (
  <button onClick={onClick} className={`px-4 py-2 rounded-lg text-[10px] font-black tracking-widest uppercase transition-all ${theme === 'dark' ? 'text-slate-400 hover:text-white hover:bg-slate-800' : 'text-slate-600 hover:text-orange-600 hover:bg-slate-100'}`}>
    {label}
  </button>
);
...
const MegaMenuButton: React.FC<{ icon: React.ReactNode; label: string; onClick: () => void; theme: string; colorClass: string }> = ({ icon, label, onClick, theme, colorClass }) => (
  <button onClick={onClick} className={`w-full flex items-center gap-3 p-2.5 rounded-xl transition-all group ${theme === 'dark' ? 'hover:bg-slate-800' : 'hover:bg-slate-50'}`}>
    ...
    <span className={`text-[11px] font-bold uppercase transition-colors ${theme === 'dark' ? 'text-slate-400 group-hover:text-white' : 'text-slate-600 group-hover:text-orange-600'}`}>
      {label}
    </span>
  </button>
);
