...
const SpamhausTool: React.FC<SpamhausToolProps> = ({ onBack, theme }) => {
  ...
  const handleProcess = async () => {
    ...
    notify('info', `Initializing blacklist check for ${uniqueInputs.length} targets.`);
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] p-4 md:p-8 flex flex-col gap-6 max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      <header className={`flex items-center justify-between p-6 rounded-3xl shadow-2xl border ${cardClasses}`}>
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-black tracking-tight uppercase">Spamhaus Blacklist</h1>
              <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest opacity-60">High-Performance IP & Domain Reputation</p>
            </div>
          </div>
        </div>
        <div className="flex gap-2">
          ...
          >
            <Info size={16} /> <span className="hidden sm:inline">Protocol</span>
          </button>
          {results.length > 0 && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-sm">
             <div className="space-y-4">
                <p className="font-bold text-slate-400 uppercase text-[10px] tracking-widest flex items-center gap-2">
                  <ChevronRight size={12} className="text-rose-500"/> Workflow
                </p>
                <ol className="list-decimal list-inside space-y-2 text-slate-400">
                  <li>Paste IPv4 addresses or domains (one per line).</li>
                  <li>Click <strong className="text-rose-400">"Run Blacklist Check"</strong>.</li>
                  <li>Analyze listed datasets and response codes.</li>
                </ol>
             </div>
             <div className="space-y-4">
                <p className="font-bold text-slate-400 uppercase text-[10px] tracking-widest flex items-center gap-2">
                  <ChevronRight size={12} className="text-rose-500"/> Dataset Dictionary
                </p>
                <ul className="space-y-3">
                  <li className="flex gap-2">
                     <span className="font-black text-rose-400 min-w-[45px]">ZEN:</span>
                     <span className="text-slate-400">Combined IP reputation (SBL, XBL, PBL).</span>
                  </li>
                  <li className="flex gap-2">
                     <span className="font-black text-rose-400 min-w-[45px]">DBL:</span>
                     <span className="text-slate-400">Domain Block List for malicious identifiers.</span>
                  </li>
                  <li className="flex gap-2">
                     <span className="font-black text-rose-400 min-w-[45px]">ZRD:</span>
                     <span className="text-slate-400">Zero Reputation Domains (New domains 0–24h).</span>
                  </li>
                </ul>
             </div>
          </div>
        ...
        <div className={`p-6 rounded-3xl shadow-xl border flex flex-col gap-4 ${cardClasses}`}>
          <div className="flex items-center justify-between">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
               <Terminal size={12} className="text-rose-500" /> Targets (IPs or Domains)
            </label>
          </div>
          <textarea
            value={input}
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-2">
            ...
            >
              {isProcessing ? <><Loader2 size={16} className="animate-spin" /> Checking Blacklist...</> : <><Zap size={16} /> Run Blacklist Check</>}
            </button>
          </div>
        </div>
      </div>
      ...
      {results.length > 0 && (
        <div className={`rounded-3xl shadow-2xl border overflow-hidden ${cardClasses}`}>
          <div className={`p-5 border-b flex flex-col md:flex-row gap-4 items-center justify-between ${isDark ? 'bg-[#05080f] border-[#1e293b]' : 'bg-slate-50 border-slate-100'}`}>
            ...
            <button 
              onClick={() => {
                const csv = [['Target', 'Listed', 'Datasets', 'Reason'].join(','), ...results.map(r => [r.input, r.listed ? 'Yes' : 'No', r.datasets.join(';'), r.reason].join(','))].join('\n');
                const blob = new Blob([csv], { type: 'text/csv' });
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.setAttribute('hidden', '');
                a.setAttribute('href', url);
                a.setAttribute('download', 'spamhaus_blacklist_audit.csv');
            ...
            <table className="w-full text-left">
              <thead>
                <tr className={`${isDark ? 'bg-[#05080f] border-[#1e293b]' : 'bg-slate-100 border-slate-200'} border-b text-[10px] font-black uppercase tracking-[0.2em] text-slate-500`}>
                  ...
                  <th className="px-6 py-4">Reputation Result</th>
                </tr>
              </thead>
                  ...
                  <tr key={r.id} className="hover:bg-rose-500/5 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                       <div className="flex flex-wrap gap-1">
                          {r.datasets.map((d, i) => (
                            <span key={i} className={`px-2 py-0.5 rounded text-[9px] font-black border uppercase tracking-tighter ${r.listed ? 'bg-rose-500/10 text-rose-500 border-rose-500/20' : 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'}`}>
                              {d}
                            </span>
                          ))}
                       </div>
                    ...
                    <td className="px-6 py-4">
                       <p className={`text-xs font-bold ${r.status === 'error' ? 'text-rose-600' : r.listed ? 'text-rose-400' : 'text-slate-500'}`}>
                         {r.reason}
                       </p>
                    </td>
                  </tr>
      ...
      <footer className={`mt-auto pt-10 border-t flex flex-col sm:flex-row justify-between items-center gap-6 text-[10px] font-black text-slate-600 uppercase tracking-widest ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
        <p>© {new Date().getFullYear()} Authenticator Pro Lab • Professional Domain Intelligence</p>
        <div className="flex gap-6">
          <a href="https://www.spamhaus.org" target="_blank" className="hover:text-rose-400 flex items-center gap-1">Official Spamhaus <ExternalLink size={10}/></a>
        </div>
      </footer>
    </div>
  );
